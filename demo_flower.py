...
from finetune import ViTLinear, inference, Trainer, ViTVPTDeep
import yaml
...
def get_config(exp_name, encoder):
    ...
    prompt_len = config.get("prompt_len", 5)  ## new added
...
def main(_):
    ...
    if net_class == ViTVPTDeep:
        model = net_class(102, FLAGS.encoder, prompt_len=prompt_len)
    else:
    ...
    print(
        f"lr: {lr:0.7f}, wd: {wd:0.7f}, best_val_acc: {best_val_acc}, best_epoch: {best_epoch}"
    )
    ...
    inference(
        test_dataloader, model, device, result_path=dir_name + "/test_predictions.txt"
    )
